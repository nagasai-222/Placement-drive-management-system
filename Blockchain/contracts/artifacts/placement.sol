...
contract HealthCare {
    ...
    event recordCreated(uint256 ID, string testName, string date, string placement, uint256 price);
    event recordSigned(uint256 ID, string testName, string date, string placement, uint256 price);

    modifier signOnly {
        require (msg.sender == Admin || msg.sender == placement, "You are not authorized to sign this.");
        _;
    }
    ...
    validateRecord(_ID) public {
        ...
        _newrecord.placement = placement;
    ...
    function signRecord(uint256 _ID) signOnly checkAuthBeforeSign(_ID) public {
        ...
        if(records.signatureCount == 2)
            emit  recordSigned(records.ID, records.testName, records.date, records.placement, records.price);
